...
export default function CustomerCareChatbot() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-45 font-sans pointer-events-none">
      ...
      {!isOpen && (
        <motion.button
          layoutId="chatbot-frame"
          onClick={() => setIsOpen(true)}
          className="pointer-events-auto flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-bold rounded-full shadow-2xl hover:shadow-emerald-600/30 transition-all cursor-pointer transform hover:scale-105 border border-emerald-500/30 group"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            layoutId="chatbot-frame"
            className={`pointer-events-auto bg-white border border-gray-150 rounded-3xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 w-80 sm:w-96 ${
          ...
          >
            ...
            <div className="bg-gradient-to-r from-neutral-900 to-neutral-800 text-white px-4 py-3.5 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  title="Close Dialog"
                  className="p-1 hover:bg-white/10 rounded-lg transition-colors cursor-pointer text-gray-300 hover:text-red-405"
                >
            ...
            {!isMinimized && (
              <>
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/70 relative">
                  ...
                  {isLoading && (
                    <div className="flex items-start gap-2 mr-auto max-w-[85%]">
                      ...
                      <div className="p-3 bg-white border border-gray-150 rounded-2xl rounded-tl-none shadow-xs flex items-center gap-1.5">
                ...
                <div className="px-3 py-2 bg-white border-t border-gray-100 flex gap-1.5 overflow-x-auto scrollbar-none shrink-0">
                  {QUICK_SUGGESTIONS.map((saj, idx) => (
                    <button
                      ...
                      className="whitespace-nowrap px-2.5 py-1 text-[10px] font-semibold bg-gray-50 hover:bg-emerald-50 text-gray-600 hover:text-emerald-800 border border-gray-150 hover:border-emerald-250 rounded-full transition-all cursor-pointer shadow-3xs shrink-0"
                    >
                      {saj}
                    </button>
                  ))}
                ...
                {/* INPUT ZONE shrink-0 */}
                <div className="p-3 bg-white border-t border-gray-150 shrink-0 flex items-center gap-2">
                  <input
                    type="text"
                    value={inputValue}
                    ...
                    className="flex-1 bg-gray-50 border border-gray-150 rounded-xl px-3.5 py-2 text-xs text-gray-700 placeholder-gray-400 focus:outline-hidden focus:border-emerald-600 focus:bg-white transition-all shadow-inner-xs"
                  />
                  <button
